...
(function($) {  
  ...
  $.fn.receiveInit = function() {
	  ...
	  var receiveViewList = $('#receiveViewList',$this);
	...
	var searchBtn = function(flag){
		...
		if(result.isSuccess){
			...
			$(receiveViewList).datagrid('loadData',datagridData);
			//需要重新重新分页信息
			if(flag){
				getTotal(content);
			}
	...
	//加载列表
	  $(receiveViewList).datagrid({
		...
		toolbar:[	
				...
				{id:'delete_'+id,text:'删除',iconCls:'icon-remove',handler:function(){onRemove();}},'-',
				{id:'print_'+id,text:'打印',iconCls:'icon-print',handler:function(){onPrint();}}
				],
		columns:[[
	...
	//内层按钮权限 
	var onInCheckRight = function(flag){
		if(flag==1){
			$('#preReceive'+id,editDialog).linkbutton('disable');
			$('#nextReceive'+id,editDialog).linkbutton('disable');
			$('#printIn'+id,editDialog).linkbutton('disable');
			$('#saveIn_'+id,editDialog).linkbutton('enable');
			$('#deleteIn_'+id,editDialog).linkbutton('disable');
		}else if(flag==2){
			var index=$(receiveViewList).datagrid("getRowIndex",selectRow);
			if(index==0){
				$('#preReceive'+id,editDialog).linkbutton('disable');
			}else{
				$('#preReceive'+id,editDialog).linkbutton('enable');
			}
			if(index==total-1){
				$('#nextReceive'+id,editDialog).linkbutton('disable');
			}else{
				$('#nextReceive'+id,editDialog).linkbutton('enable');
			}
			
			if($('#print_'+id,$this).linkbutton('options').disabled){
				$('#printIn'+id,editDialog).linkbutton('disable');
			}else{
				$('#printIn'+id,editDialog).linkbutton('enable');
			}
			if($('#add_'+id,$this).linkbutton('options').disabled){
				$('#addIn_'+id,editDialog).linkbutton('disable');
			}else{
				$('#addIn_'+id,editDialog).linkbutton('enable');
			}
			if($('#delete_'+id,$this).linkbutton('options').disabled){
				$('#deleteIn_'+id,editDialog).linkbutton('disable');
			}else{
				$('#deleteIn_'+id,editDialog).linkbutton('enable');
			}
		}
	};
	...
	var onRemove = function(){
		var rows = $(receiveViewList).datagrid('getChecked');
		if(rows.length==0){
	...
	$(editDialog).dialog({  
		...
		{id:'printIn'+id,text:'打印',iconCls:'icon-print',handler:function(){onPrintIn();}},'-',
		{
			id:'addIn_'+id,
			text:'新增',
			iconCls:'icon-add',
	...
	function setValue(){
		...
		if(recRejDate==""||recRejDate==null){
			$.messager.alert('提示','请填写入库日期','error');
			return false;
		}
		//供应商
	...
	var onPrint = function(){
		...
		print(selectRow.recRejId);
	};
	//打印
	var print = function(recRejId){
		window.open("printReport.jsp?report=recRej&data=ReportServlet?recRejId="+recRejId);
	};
	//内层打印
	var onPrintIn = function(){
		var recRejId = $('#recRejId',editDialog).val();
		print(recRejId);
	};
	//保存
				...
				if(result.isSuccess){
					var fn = function(){
						...
						if(recRejId==''){//新增
							...
							var options = $('#update_'+id,$this).linkbutton('options');
							if(options.disabled){
								$('#saveIn_'+id,editDialog).linkbutton('disable');
							}
						}
	...
	function onOpenIndex(index){
		...
		$(receiveViewList).datagrid('unselectRow',selectIndex);
		$(receiveViewList).datagrid('selectRow',selectIndex+index);
