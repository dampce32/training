...
(function($) {  
  ...
  $.fn.feeItemInit = function() {
	...
	var setValue = function(){
		var feeItemName = $.trim($('#feeItemName',editForm).val());
		if(''==feeItemName){
			$.messager.alert('提示','请填写消费项','warning');
			return false;
		}
		return true;
	};
	...
	var onSave = function(){
		$(editForm).form('submit',{
			...
			success: function(data){
				...
				if(result.isSuccess){
					var fn = function(){
						var feeItemId = $.trim($('#feeItemId',editForm).val());
						if(feeItemId==''){//新增
							var  data = result.data;
							$('#feeItemId',editForm).val(data.feeItemId);
						}
						TRN.FeeItemList = null;
					};
	...
	var onDelete = function(){
		...
		$.messager.confirm('确认', '确定删除该记录吗?', function(r){
			if(r){
				var url = "dict/deleteFeeItem.do";
				var content = {feeItemId:selectRow.feeItemId};
				$.post(url,content,
					function(result){
						if(result.isSuccess){
							search(true);
	...
	var search = function(flag){
		var feeItemName = $('#feeItemNameSearch',queryContent).val();
		var content = {feeItemName:feeItemName,page:pageNumber,rows:pageSize};
		//取得列表信息
	...
	var onMulUpdateStatus = function(status){
		...
		$.messager.confirm("提示","确定要"+msg+"记录?",function(t){ 
			if(t){
				var url = 'dict/updateStatusFeeItem.do';
				var content ={feeItemId:selectRow.feeItemId,status:status};
